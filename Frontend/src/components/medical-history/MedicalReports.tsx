...
const MedicalReports = () => {
  ...
  const handleUpload = async (reportData: MedicalReport) => {
    console.log('New report data:', reportData);
    // Add the new report to the list immediately
    setReports(prevReports => [reportData, ...prevReports]);
    // Refresh the reports list from server
    await fetchReports();
  };
  ...
  const handleDownload = async (report: MedicalReport) => {
    try {
      ...
      link.href = url;
      link.download = report.originalName; // Use the original filename
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Type: {report.reportType.charAt(0).toUpperCase() + report.reportType.slice(1)}
                      </p>
                    </div>
                  </div>
